...
def load_data(path, dataset):   #path="../data/citeseer/", dataset="citeseer"
...
def load_data1(path, dataset, cuda=True):  # path="../data/citeseer/", dataset="citeseer"
    print('Loading {} dataset...'.format(dataset))
    features_block = False  # concatenate the feature spaces or not
    ...
    adj_fin = sp.coo_matrix(normalize_adj(adj+sp.eye(adj.shape[0])))

    adj_all_0 = sp.coo_matrix(adj_all)
    """after this give   csr_matrix (0, 0)	0.5
    (10669, 0)	0.013964519336948639
    (10672, 0)	0.014125195547029415"""

    #adj_1 = preprocess_citation(adj)

    adj_1 = sparse_mx_to_torch_sparse_tensor1(adj_all_0).float()   #change to adj_1 not directly use adj
    adj_fin_0 = sparse_mx_to_torch_sparse_tensor1(adj_fin).float()
    if cuda:
        adj_fin_0 = adj_fin_0.cuda()
    ...
    # adj_list1 = sparse_mx_to_torch_sparse_tensor1(adj)
    # adj_label = get_A_r(adj_list1, 2)
    # adj_label1 = torch.FloatTensor(adj_label)

    return adj_list, features_list, Labels, idx_train, idx_val, idx_test, idx_map_list[0], adj_fin_0, features #not retun adj_1
...
def sparse_mx_to_torch_sparse_tensor1(sparse_mx):
    """Convert a scipy sparse matrix to a torch sparse tensor."""
    sparse_mx = sparse_mx.tocoo().astype(np.float32)
    indices = torch.from_numpy(
        np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64))
