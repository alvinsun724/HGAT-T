...
class Mlp1(nn.Module):   #this is one of the difficult part can mention
    def __init__(self, in_dim, hid_dim, dropout):
        ...
        self.fc1 = Linear(4279, hid_dim)   ###need to check again, Mr =4279
        self.fc2 = Linear(hid_dim, hid_dim)
        self.act_fn = torch.nn.functional.gelu
        self._init_weights()
        self.dropout = Dropout(dropout)
    ...
    def forward(self, x):
        x = self.fc1(x)    #argument 'input' (position 1) must be Tensor, not list
        x = self.act_fn(x)
        x = self.layernorm(x)
        x = self.dropout(x)
        x = self.fc2(x)
...
class HGAT1(nn.Module):
    def __init__(self, nfeat_list, nhid, nclass, dropout,
        ...
        self.mlp = Mlp1(4279, nhid, dropout)    #mr is 4279
    ...
    def forward(self, x_list, adj_list, adj_all=None):
        x0 = x_list
        x_d = self.mlp(x0[0]) #10662
        x_dis = get_feature_dis(x_d)  #newly add, is 10662, different from later 6106 adj
