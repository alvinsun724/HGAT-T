...
def train(epoch,
    ...
    output,x_dis = model(adj_label_train_batch, input_features_train_batch)
...
def test1(epoch, input_adj_test, input_features_test, idx_out_test, idx_test):
    print(' '*90 if 'multi' in dataset else ' '*65, end='')
    t = time.time()
    model.eval()
    output = model(input_features_test, input_adj_test)
...
# Model and optimizer
    print("\n\nNo. {} test.\n".format(i+1))
    model = HGAT1(nfeat = feat.shape[1], nfeat_list=[i.shape[1] for i in features],   #nfeat is 3555
