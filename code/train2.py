...
from util import load_data, accuracy, dense_tensor_to_sparse, resample, makedirs, load_data1
from utils_inductive import transform_dataset_by_idx
from models1 import HGAT1
...
parser.add_argument('--order', type=int, default=1,        #change from 2
...
loss_list = dict()
path = '../data/'+ dataset +'/'
adj, features, labels, idx_train_ori, idx_val_ori, idx_test_ori, idx_map, adj1, feat = load_data1(path = path, dataset = dataset, cuda=True)
##change to load_data1
tmp = feat.to_dense() #number of node 6106, tmp.shape[0];features dimension of MR is 3555, tmp.shape[1]
N = len(adj)

if torch.cuda.is_available():
    print('using cuda')
    adj_label1 = adj1.cuda()
    feat_fin = feat.cuda()
...
def get_batch(adj_label1, feat):
    adj_label1 = adj_label1.to_dense()
    feat_fin = feat.to_dense()
    rand_indx = torch.tensor(np.random.choice(np.arange(adj_label1.shape[0]), len(idx_train))).type(torch.long).cuda()
    input_features_train_batch = feat_fin[rand_indx]
    adj_label_train_batch = adj_label1[rand_indx, :][:, rand_indx]
    return input_features_train_batch, adj_label_train_batch
...
def train(epoch,
          adj_label1, features, idx_out_train, idx_train,
          input_adj_val, input_features_val, idx_out_val, idx_val):   #change input_adj_train and input_features_train
    print('Epoch: {:04d}'.format(epoch+1), end='')
    t = time.time()
    input_features_train_batch, adj_label_train_batch = get_batch(adj1, feat)
    model1.train()
    optimizer.zero_grad()
    #output = model1(input_features_train, input_adj_train)
    output,x_dis = model1(adj_label_train_batch, input_features_train_batch)
    ...
    loss_Ncontrast = Ncontrast(x_dis, adj_label_train_batch, tau=args.tau)

    loss_train = loss_train + loss_Ncontrast * args.alpha
    ...
    loss_list[epoch] = [loss_train.item()] #add from down model.eval as comment it because of val

# ---------- comment as already set test1 function for the val
    # model.eval()
...
def test(epoch, input_adj_test, input_features_test, idx_out_test, idx_test):
    ...
    print(' | time: {:.4f}s'.format(time.time() - t))
    #*loss_list[epoch] += [loss_test.item()]   comment it on 11.12 for running
...
def test1(epoch, input_adj_val, input_features_val, idx_out_val, idx_val):
    t = time.time()
    model.eval()
    output = model(input_features_val, input_adj_val)
...
# Model and optimizer
    print("\n\nNo. {} test.\n".format(i+1))
    model1 = HGAT1(nfeat = feat.shape[1], nfeat_list=[i.shape[1] for i in features],   #nfeat is 3555
    ...
    print(len(list(model1.parameters())))
    print([i.size() for i in model1.parameters()])
    # Train model
    t_total = time.time()
    vali_max = [0, [0, 0], -1]
...
####################################
    ...
    for epoch in range(args.epochs):#change vali_acc, vali_f1 = train()
...
"""
adj_label1 = adj_label1.to_dense()
feat_fin = feat_fin.to_dense()

#input_features_train_0 = input_features_train.to_dense()
input_features_train_0 = feat.to_dense()
#*adj_label2 = adj_label1.detach().to("cpu").numpy()
rand_indx = torch.tensor(np.random.choice(np.arange(adj_label1.shape[0]),len(idx_train))).type(torch.long).cuda()
#if not put 2048, the rand_indx will has no dimension, guess as torch.cuda.sparse tensor so problem:      can't convert cuda:0 device type tensor to numpy. Use Tensor.cpu() to copy the tensor to host memory first
#id = torch.tensor(np.arange(idx_train.shape[0])).type(torch.long).cuda()
#adj_label_train_batch = adj_label1[id, :][:, id]  #*need double check sparse tensors do not have strides

#**rand_indx[0: idx_train.shape[0]] = idx_train
input_features_train_batch = input_features_train_0[rand_indx]
adj_label_train_batch = adj_label1[rand_indx,:][:,rand_indx]
"""