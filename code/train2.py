...
loss_list = dict()
path = '../data/'+ dataset +'/'
adj, features, labels, idx_train_ori, idx_val_ori, idx_test_ori, idx_map, adj1 = load_data1(path = path, dataset = dataset)
...
adj_label1 = get_A_r(adj_label1, args.order)
...
if args.cuda:
    ...
    # *rand_indx = torch.tensor(np.random.choice(np.arange(adj_label1), 2048)).type(torch.long).cuda()
    ...
    # *input_features_train_batch = input_features_train[rand_indx]
    # *adj_label_train_batch = adj_label1[rand_indx,:][:,rand_indx]


    index = adj_label1.multinomial(num_samples=len(idx_train), replacement=True)
    input_features_train_batch =input_features_train[index]
    adj_label_train_batch = adj_label1[index,:][:,index]
...
def train(epoch,
          adj_label_train_batch, input_features_train_batch, idx_out_train, idx_train,
