...
if args.cuda:
    ...
    #adj_label1_train = adj_label1
    rand_indx = torch.tensor(np.random.choice(np.arange(adj_label1.shape[0]))).type(torch.long).cuda()
    rand_indx_1 = torch.tensor(np.arange(idx_train.shape[0])).type(torch.long).cuda()
    id = torch.tensor(np.arange(idx_train.shape[0])).type(torch.long).cuda()
    adj_label1 = adj_label1.to_dense()

    rand_indx[0:idx_train.shape[0]] = rand_indx_1
    #rand_indx[0:len(idx_train)] = len(idx_train)
    #adj_label1_train = adj_label1[rand_indx, :][:, rand_indx]
...
def train(epoch,
    ...
    loss_Ncontrast = Ncontrast(x_dis, adj_label1_train, tau=args.tau)
